...
namespace LLMUnitySamples
{
    ...
    class Bubble
    {
        ...
        {
            Debug.Log($"Chatbot_debug: 🟢 Setting InputBubble text to: {text}");
    ...
    class InputBubble : Bubble
    {
        ...
        {
            Debug.Log($"Chatbot_debug: 🆕 New InputBubble created with message: {message}");
        ...
        {
            //Debug.Log("Chatbot_debug: 🔄 Entering FixCaretSorting()");

            GameObject caret = GameObject.Find($"{inputField.name} Input Caret");
            if (caret == null)
            {
               // Debug.LogError("Chatbot_debug: ❌ Care t not found!");
                return; // Prevents a NullReferenceException
            }

            Canvas bubbleCanvas = caret.GetComponent<Canvas>();
            if (bubbleCanvas == null)
            {
                Debug.Log("Chatbot_debug: 🛠️ Adding Canvas to caret...");
                bubbleCanvas = caret.AddComponent<Canvas>();
                bubbleCanvas.overrideSorting = true;
                bubbleCanvas.sortingOrder = 3;
            }

            //Debug.Log("Chatbot_debug: ✅ FixCaretSorting completed.");
        }
        ...
        //new text setting part 1
        public void SafeSetText(string newText, MonoBehaviour owner)
        {
            owner.StartCoroutine(DelayedSetText(newText));
        }
        //new text setting part 2
        private IEnumerator DelayedSetText(string newText)
        {
            while (inputField == null || !inputField.isActiveAndEnabled)
            {
                yield return null;
            }

            inputField.text = newText;
            Debug.Log($"Chatbot_debug: 🕒 DelayedSetText now setting: {newText}");
        ...
        // flag added to override the set text method in bubble class to get mroe info 
        public new void SetText(string newText)
        {
            Debug.Log($"Chatbot_debug: ✏ Attempting to set InputBubble text to: {newText}");

            if (inputField == null)
            {
                Debug.LogError("Chatbot_debug: ❌ inputField is NULL in SetText!");
                return;
            }

            Debug.Log($"Chatbot_debug: 🔍 InputField exists. Current text before setting: '{inputField.text}'");
            Debug.Log($"Chatbot_debug: test");
            Debug.Log($"Chatbot_debug: 🔍 InputField enabled: {inputField.enabled}, activeInHierarchy: {inputField.gameObject.activeInHierarchy}");


            inputField.text = newText;  // This is where it might be getting stuck
            Debug.Log($"Chatbot_debug: moved to end");
            MoveTextEnd();
            Debug.Log($"Chatbot_debug: ✅ InputField text set successfully to: '{inputField.text}'");
        }
        ...
        {
            Debug.Log($"Chatbot_debug: 🟡 Setting placeholder text to: {text}");
        ...
        {
            //Debug.Log("Chatbot_debug: 🔄 Entering ActivateInputField()");
            if (inputField == null)
            {
                //Debug.LogError("Chatbot_debug: ❌ inputField is NULL in ActivateInputField()!");
                return;
            }

            if (!inputField.interactable)
            {
              //  Debug.LogError("Chatbot_debug: ❌ Input field is NOT interactable!");
            }
            else
            {
               // Debug.Log("Chatbot_debug: ✅ Input field is interactable.");
            }

            //inputField.interactable = true; // ✅ Force it to be interactable before activating

            inputField.ActivateInputField();
            //Debug.Log("Chatbot_debug: 🔍 Calling FixCaretSorting...");
            FixCaretSorting();
           // Debug.Log("Chatbot_debug: ✅ FixCaretSorting completed!");

           // Debug.Log("Chatbot_debug: ✅ inputField activation completed.");
        }
        ...
        {
            Debug.Log("Chatbot_debug: 🔄 Entering reActivateInputField()");
            if (inputField == null)
            {
                Debug.LogError("Chatbot_debug: ❌ inputField is NULL inside reActivateInputField()!");
                return;
            }
            Debug.Log("Chatbot_debug: 🎯 Setting inputField to selected & focused...");


            inputField.DeactivateInputField();
            Debug.Log("Chatbot_debug: DeactivateInputFielded");

            inputField.Select();
            Debug.Log("Chatbot_debug: Selected");

            inputField.ActivateInputField();
            Debug.Log("Chatbot_debug: ✅ inputField reactivation completed.");
